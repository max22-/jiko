...
static token_t *open_bracket(lexer_t *lex) {
    token_t *res = token_new(TOK_OPEN_BRACKET, "[", lex->start_line, lex->start_column);
    next_char(lex);
    update_start(lex);
    return res;
}
...
static token_t *close_bracket(lexer_t *lex) {
    token_t *res = token_new(TOK_CLOSE_BRACKET, "]", lex->start_line, lex->start_column);
    next_char(lex);
    update_start(lex);
    return res;
}
...
static token_t *integer(lexer_t *lex) {
    ...
    token_t *res = token_new(TOK_INTEGER, strval, lex->start_line, lex->start_column);
    update_start(lex);
    free(strval);
    return res;
}
...
static token_t *string(lexer_t *lex) {
    ...
    while(1) {
        if(lex->pos >= lex->text_len)
            return token_new(TOK_ERROR, "Unexpected EOF in string literal", lex->start_line, lex->start_column);
        if(lex->text[lex->pos] == '"')
            break;
        if(lex->text[lex->pos] == '\\') {
            next_char(lex);
            ...
            case '\\':
                break;
            default:
                return token_new(TOK_ERROR, "Invalid escaped character", lex->start_line, lex->start_column);
            }
        }
        next_char(lex);
    }
    ...
    next_char(lex);
    char *strval = strndup(&lex->text[lex->start], lex->pos - lex->start);
    token_t *res = token_new(TOK_STRING, strval, lex->start_line, lex->start_column);
    update_start(lex);
    free(strval);
    return res;
}
...
static token_t *word(lexer_t *lex) {
    ...
    char *strval = strndup(&lex->text[lex->start], lex->pos - lex->start);
    token_t *res = token_new(TOK_WORD, strval, lex->start_line, lex->start_column);
    lex->start = lex->pos;
    free(strval);
    return res;
